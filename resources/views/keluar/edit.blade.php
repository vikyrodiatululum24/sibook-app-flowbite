<x-app-layout>
    ...
    <div class="py-6 sm:ml-64">
        <div class="p-4 flex-col max-w-xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow">
            ...
            @endif
            <form class="max-w-full" method="POST" action="{{ route('keluar.update', $keluar->id) }}">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label for="buku_id"
                    ...
                    <select id="buku_id" name="buku_id"
                        ...
                        required>
                        @if (old('buku_id', $keluar->buku_id))
                            <option value="{{ old('buku_id', $keluar->buku_id) }}" selected>
                                {{ $keluar->buku_id ? $keluar->buku->name : 'Customer Terpilih' }}
                            </option>
                        @endif
                    </select>
                </div>
                <div class="mb-4">
                    ...
                    <select id="customer_id" name="customer_id"
                        class="form-select select2 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        required>
                        @if (old('customer_id', $keluar->customer_id))
                            <option value="{{ old('customer_id', $keluar->customer_id) }}" selected>
                                {{ $keluar->customer ? $keluar->customer->name : 'Customer Terpilih' }}
                            </option>
                        @endif
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="date" id="tanggal_keluar" name="tanggal_keluar"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        required
                        value="{{ old('tanggal_keluar', $keluar->tanggal_keluar ? \Carbon\Carbon::parse($keluar->tanggal_keluar)->format('Y-m-d') : '') }}" />
                </div>
                <div class="mb-4">
                    ...
                    <select id="status_keluar" name="status_keluar"
                        ...
                        <option value="" disabled
                            {{ old('status_keluar', $keluar->status_keluar) == '' ? 'selected' : '' }}>Pilih Status
                        </option>
                        <option value="terjual"
                            {{ old('status_keluar', $keluar->status_keluar) == 'terjual' ? 'selected' : '' }}>Terjual
                        </option>
                        <option value="sample"
                            {{ old('status_keluar', $keluar->status_keluar) == 'sample' ? 'selected' : '' }}>Sample
                        </option>
                        <option value="lainnya"
                            {{ old('status_keluar', $keluar->status_keluar) == 'lainnya' ? 'selected' : '' }}>Lainnya
                        </option>
                    </select>
                </div>
                <div class="mb-4">
                    ...
                    <textarea id="keterangan" name="keterangan"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                        placeholder="Masukan Keterangan">{{ old('keterangan', $keluar->keterangan) }}</textarea>
                </div>

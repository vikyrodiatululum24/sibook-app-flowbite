            ...
            <form class="max-w-full" method="POST" action="{{ route('keluar.update', $keluar->id) }}">
              ...
              <div class="mb-4">
                ...
                <select id="buku_id" name="buku_id"
                  ...
                  required>
                  @if($keluar->buku)
                    <option value="{{ $keluar->buku->id }}" selected>{{ $keluar->buku->name }}</option>
                  @else
              ...
              <div class="mb-4">
                ...
                <select id="customer_id" name="customer_id"
                  ...
                  required>
                  @if($keluar->customer)
                    <option value="{{ $keluar->customer->id }}" selected>{{ $keluar->customer->name }}</option>
                  @else
                    <option value="" disabled selected></option>
                  @endif
                </select>
              ...
              <div class="mb-4">
                ...
                <input type="date" id="tanggal_keluar" name="tanggal_keluar"
                  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                  required value="{{ old('tanggal_keluar', $keluar->tanggal_keluar ? \Carbon\Carbon::parse($keluar->tanggal_keluar)->format('Y-m-d') : '') }}" />
              </div>
              <div class="mb-4">
                ...
                <select id="status_keluar" name="status_keluar"
                  ...
                  required>
                  <option value="" disabled {{ old('status_keluar', $keluar->status_keluar) == '' ? 'selected' : '' }}>Pilih Status</option>
                  <option value="terjual" {{ old('status_keluar', $keluar->status_keluar) == 'terjual' ? 'selected' : '' }}>Terjual</option>
                  <option value="sample" {{ old('status_keluar', $keluar->status_keluar) == 'sample' ? 'selected' : '' }}>Sample</option>
                  <option value="lainnya" {{ old('status_keluar', $keluar->status_keluar) == 'lainnya' ? 'selected' : '' }}>Lainnya</option>
                </select>
              </div>
              <div class="mb-4">

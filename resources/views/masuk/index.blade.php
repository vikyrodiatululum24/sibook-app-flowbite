            ...
            <tbody>
                @foreach ($masuks as $masuk)
                    <tr>
                        ...
                        <td class="flex gap-2">
                            <!-- View -->
                            <a href="{{ route('masuk.show', $masuk->id) }}" class="text-blue-500 hover:text-blue-700"
                                title="Lihat">
                            ...
                            <!-- Delete -->
                            <form id="form-delete-{{ $masuk->id }}"
                                action="{{ route('masuk.destroy', $masuk->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="button"
                                    class="btn-delete text-red-500 px-3 hover:text-red-700 rounded hover:cursor-pointer"
                                    data-id="{{ $masuk->id }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 inline" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
